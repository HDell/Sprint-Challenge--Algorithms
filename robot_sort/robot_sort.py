    ...
    def sort(self): #selection sort
        ...
        while self.can_move_right():
            if self.compare_item() == -1:
                ...
                self.swap_item() # pick up the None
                if self.can_move_left(): # move the None one behind its current position
                    self.move_left()
                    self.swap_item()
                    self.move_to_beginning() #start over
                else: # None was already at the front so the list is sorted
                    return
            else:
                self.move_right()
            if not self.can_move_right() and self.compare_item() is None: #first time the end is reached
                self.swap_item() # pick up the None
                if self.can_move_left(): # move the None one behind its current position
                    self.move_left()
                    self.swap_item()
